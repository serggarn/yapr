...
model::Game LoadGame(const std::filesystem::path& json_path) {
    ...
    model::Game game;
    auto jsn_obj = jsn_values.as_object();
    auto default_dog_speed = jsn_obj.contains(json_tags::defaultDogSpeed)
            ? jsn_obj.at(json_tags::defaultDogSpeed).as_double()
            : model::Game::default_dog_speed;
    auto default_bag_capacity = jsn_obj.contains(json_tags::defaultBagCapacity)
                ? jsn_obj.at(json_tags::defaultDogSpeed).as_double()
...
model::Map LoadMap(const json::value& jsn_value,
    ...
    auto dg_spd = jsn_obj.contains(dogSpeed) ?
                  jsn_obj.at(dogSpeed).as_double() : default_dog_speed;
    auto bag_capacity = jsn_obj.contains(bagCapacity) ?
                  jsn_obj.at(bagCapacity).as_uint64() : default_bag_capacity;
