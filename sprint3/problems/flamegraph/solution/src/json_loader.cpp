...
model::Game LoadGame(const std::filesystem::path& json_path) {
	...
	for ( auto const& jsn_map : jsn_maps ) {
		...
		for ( auto const& jsn_bldg : jsn_bldngs ) {
			model::Point point { json::value_to<model::Coord>(jsn_bldg.as_object().at("x")), json::value_to<model::Coord>(jsn_bldg.as_object().at("y")) };
			model::Size size { json::value_to<model::Dimension>(jsn_bldg.as_object().at("w")),  json::value_to<model::Dimension>(jsn_bldg.as_object().at("h")) };
		...
		for ( auto const& jsn_ofc : jsn_ofcs ) {
			...
			model::Point position { json::value_to<model::Coord>(jsn_ofc.as_object().at("x")), json::value_to<model::Coord>(jsn_ofc.as_object().at("y")) };
			model::Offset offset { json::value_to<model::Coord>(jsn_ofc.as_object().at("offsetX")), json::value_to<model::Coord>(jsn_ofc.as_object().at("offsetY")) };
