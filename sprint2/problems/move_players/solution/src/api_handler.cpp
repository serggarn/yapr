...
StringResponse ApiHandler::MakeUnauthorizedError(const StringRequest& request) {
    ...
    std::string_view body = request.method() == http::verb::head ? "" : json::serialize(answ_obj);
    return MakeStringResponse(status, body, request.version(), request.keep_alive());
}
...
StringResponse ApiHandler::MakeUnallowedMethodError(const StringRequest& request, std::string_view allow_method) {
    ...
    std::string_view body = request.method() == http::verb::head ? "" : json::serialize(answ_obj);//answer.c_str();
    return MakeStringResponse(status, body, request.version(), request.keep_alive(),
